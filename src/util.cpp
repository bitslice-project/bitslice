...
int nAnonymizeBitsliceAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitslice
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitslice
    // Mac: ~/Library/Application Support/Bitslice
    // Unix: ~/.Bitslice
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitslice";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Bitslice.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bitsliced.pid"));
