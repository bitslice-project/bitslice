...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBitsliceAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBitsliceAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeBitsliceAmount = strAnonymizeBitsliceAmount.remove(strAnonymizeBitsliceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitsliceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeBitsliceAmount*COIN) nMaxToAnonymize = nAnonymizeBitsliceAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeBitsliceAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeBitsliceAmount));
        strAnonymizeBitsliceAmount = strAnonymizeBitsliceAmount.remove(strAnonymizeBitsliceAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBitsliceAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
