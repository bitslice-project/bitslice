...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBitsliceAmount"))
        settings.setValue("nAnonymizeBitsliceAmount", 1000);
    nAnonymizeBitsliceAmount = settings.value("nAnonymizeBitsliceAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeBitsliceAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeBitsliceAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeBitsliceAmount:
            return QVariant(nAnonymizeBitsliceAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeBitsliceAmount:
            nAnonymizeBitsliceAmount = value.toInt();
            settings.setValue("nAnonymizeBitsliceAmount", nAnonymizeBitsliceAmount);
            emit AnonymizeBitsliceAmountChanged(nAnonymizeBitsliceAmount);
            break;
        default:
            break;
        }
